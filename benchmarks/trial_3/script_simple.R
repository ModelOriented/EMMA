...
pipes_simple_num <- c(PipeOpImputeMean, PipeOpImputeMedian, PipeOpImputeHist, PipeOpImputeSample, PipeOpImputeOOR)
...
pipes_simple_num <- c(PipeOpMean_B, PipeOpMedian_B, PipeOpHist_B, PipeOpSample_B, PipeOpOOR_B)
...
for (task_id in tasks$task.id[1]) {
  
  for (j in c(4)) {
    
    for (model in learners[c(4)]) {
