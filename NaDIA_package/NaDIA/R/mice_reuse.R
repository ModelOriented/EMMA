...
mice.reuse <- function(mids, newdata, maxit = 5, printFlag = TRUE, seed = NA) {
  ...
  mids.comb <- NaDIA::mids.append(mids, mids.new)
  ...
  for (j in names(mids.comb$imp)) {
    for (i in seq_len(mids.comb$m)) {
      mids.comb$imp[[j]][, i] <-
        NaDIA::replace_overimputes(actual_data, mids.comb$imp, j, i)
    }
  }
  ...
  cond_imp <- "imp[[j]][, i] <- NaDIA::replace_overimputes(NaDIA::fetch_data(), imp, j, i)"
