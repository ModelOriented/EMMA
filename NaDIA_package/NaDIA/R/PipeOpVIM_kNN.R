...
PipeOpVIM_kNN <- R6::R6Class("VIM_kNN_imputation",
  ...
  public = list(
    initialize = function(id = "impute_VIM_kNN_B", k = 5, numFun = median, catFun = VIM::maxCat, out_file = NULL) {
      super$initialize(id,
        whole_task_dependent = TRUE, packages = "NaDIA", param_vals = list(k = k, numFun = numFun, catFun = catFun, out_file = out_file),
    ...
    .train_imputer = function(feature, type, context) {
      ...
      imp_function <- function(data_to_impute) {
        ...
        data_imputed <- NaDIA::autotune_VIM_kNN(data_to_impute, percent_of_missing,
