...
to_plot <- data
to_plot$per_variables_miss <- round(100*(to_plot$no_cat_w_miss+to_plot$no_num_w_miss)/to_plot$no_features)

ggplot(to_plot, aes(per_variables_miss))+
  geom_histogram(aes(y = stat(count/sum(count))), fill='#2b8cbe', breaks = seq(0, 100, 5))+
