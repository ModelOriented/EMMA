...
context('Testing full workflow with learners')
  ...
  imps <- c(PipeOpAmelia$new(),PipeOpmissForest$new(),PipeOpMissMDA_PCA_MCA_FMAD$new(optimize_ncp = F),
  ...
  test_that(paste0('Testing imputation with learner ',i$id),{
    ...
    graph <- i %>>% PipeOpEncodeImpact$new() %>>% lrn('classif.glmnet')
    ...
    else{expect_is(resample(task,graph_learner,rsmp('cv',folds=2)),'ResampleResult')}
  ...
  test <- as.data.frame(cbind(as.data.frame(tsk('pima')$data()),as.integer(sample(c(1:10,NA),768,replace = T)),as.factor(sample(c('a','b','c','d',NA),768,replace = T))))
  ...
  task <- TaskClassif$new('test',test,'diabetes')


  imps <- c(PipeOpMice$new(),PipeOpMice_A$new())
  for (i in imps){
